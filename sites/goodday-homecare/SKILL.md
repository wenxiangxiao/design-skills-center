...
- **角色識別（多角色系統必做）**：每個角色端指定一個 `--role` 色相，寫在 `body[data-role]` 上，全站的按鈕、現用態、進度條、dock 標記、focus 環都吃這個變數。頁首上緣壓一條 5px 該色實線、右側放一枚 `.rolebadge` 圓角徽章（圖示＋角色名＋Caveat 英文旁白）。**同一套設計系統、不同色相**——換色不換骨架，才不會變成四個不同的產品。
- **頁面說明條 `.pagenote`**：頁首下方一條 `--role-soft` 底色的窄帶，左側粗體寫「誰在用」、右側說明「這頁在做什麼」。給不熟系統的人（客戶、新進員工）定位用。
- **分頁籤 `.tabs`**：作業端一頁塞多件事時，用編號圓點＋標題的頁籤拆開，一次只顯示一個 `.sec`，現用籤以 3px `--role` 底線標記。**必須支援 hash 深連結**（`#match` 直接開在該籤），簡報與客服才能把人導到正確的那一段。
- **表格數字欄**：金額、里程、時間等「一個完整的值」加 `class="num"`（`tabular-nums` ＋ `white-space:nowrap`），並把表格包進 `.tbwrap`（`overflow-x:auto`）——不折行、窄螢幕改橫向捲動。
...
- 介面文案用業務聽得懂的話當主詞，技術名詞放括號或另闢對照表（「自動排出最省車程的順序」而非「2-opt 最佳化」）。多角色系統應附一頁總覽，說明每個角色看得到什麼、資料如何串接。
